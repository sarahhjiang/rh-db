...
def plot_donors_per_state(db_url):
    ...
    # Ensure the columns are uppercase and aligned
    ...
    gdf['num_donors'] = gdf['num_donors'].fillna(0)

    # Create a color column based on the number of donors
    def make_color_column(gdf, variable, vmin, vmax):
        norm = mcolors.Normalize(vmin=vmin, vmax=vmax, clip=True)
        mapper = plt.cm.ScalarMappable(norm=norm, cmap=plt.cm.Blues)
        gdf['color'] = gdf[variable].apply(lambda x: mcolors.to_hex(mapper.to_rgba(x)) if pd.notnull(x) else '#FFFFFF')
        return gdf

    variable = 'num_donors'
    vmin, vmax = gdf[variable].min(), gdf[variable].max()
    gdf = make_color_column(gdf, variable, vmin, vmax)
    ...
    ax.axis('off')
    ax.set_title('Number of Donors per State', fontsize=14, fontweight='bold')

    # Plot the states
    gdf.plot(color=gdf['color'], linewidth=0.8, ax=ax, edgecolor='0.8')
    ...
    # Save the plot
    plot_path = os.path.join('static', 'donors_per_state_heatmap.png')
