...
class TrackerDonorDevices(db.Model):
    ...
    DeviceSerialNumber = db.Column(db.String, unique=True, nullable=False)

    # Relationship for device models
    device = db.relationship('DeviceModels', overlaps='device_model, tracker_donor_devices', back_populates='donations')

    @staticmethod
    def generate_tracking_number():
        """Generate a simple sequential tracking number."""
        count = db.session.query(db.func.count(TrackerDonorDevices.TrackerDonorDevicesKey)).scalar() or 0
        return f"{count + 1}"

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        if not self.DeviceSerialNumber:
            self.DeviceSerialNumber = self.generate_tracking_number()

    def __repr__(self):
        return f'<TrackerDonorDevices {self.DeviceSerialNumber}>'
