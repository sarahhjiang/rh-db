...
def search_results():
    ...
    if search_type == 'donors':
        ...
        results = (
            db.session.query(TrackerDonorDevices, DeviceModels, TrackerDonors)
            ...
            .filter(DeviceModels.DeviceModelName.ilike(f'%{query}%'))
            .all()
        )
...
def add_device():
    if request.method == 'POST':
        serial_number = request.form.get('DeviceSerialNumber') or str(uuid.uuid4())
            ...
            try:
                ...
                if table_name == 'donors':
                    for index, row in df.iterrows():
                        ...
                        db.session.add(new_donor)
                        print('donors successfully added')
                    ...
                    for index, row in df.iterrows():
                        ...
                        db.session.add(new_org)
                        print('organizations successfully added')
                    ...
                    for index, row in df.iterrows():
                        ...
                        print('donor devices successfully added')
    ...
    if organization_ids:
        for org_id in organization_ids:
            ...
            if organization:
                db.session.delete(organization)
        db.session.commit()
        flash('Selected organizations have been deleted successfully.', 'success')
    else:
        flash('No organizations were selected for deletion.', 'warning')
...
@app.route('/organization_requests/<int:org_id>', methods=['GET', 'POST'])
@login_required
def organization_requests(org_id):
    organization = Organization.query.get_or_404(org_id)

    if request.method == 'POST':  # Handle adding a device to a request
        serial_number = request.form.get('serial_number')
        request_id = request.form.get('request_id')

        if not serial_number or not request_id:
            flash('Serial number and request description are required.', 'danger')
            return redirect(url_for('organization_requests', org_id=org_id))

        # Fetch the device by serial number
        device = TrackerDonorDevices.query.filter_by(DeviceSerialNumber=serial_number).first()

        if not device:
            flash('Device with the specified serial number does not exist.', 'danger')
            return redirect(url_for('organization_requests', org_id=org_id))

        # Update the device with the new request assignment
        device.OrganizationProgramKey = request_id
        device.TrackerDonationDateSentOut = datetime.utcnow()  # Update the sent-out date

        db.session.commit()
        flash('Device added to request successfully.', 'success')
        return redirect(url_for('organization_requests', org_id=org_id))

    # GET request: Fetch all requests and their associated devices
    requests = OrganizationProgram.query.filter_by(OrganizationKey=org_id).all()
    requests_with_devices = [
        {
            'request': req,
            'devices': TrackerDonorDevices.query.filter_by(OrganizationProgramKey=req.OrganizationProgramKey).all()
        }
        for req in requests
    ]

    # Fetch devices that are not currently assigned to any request
    available_devices = TrackerDonorDevices.query.filter_by(OrganizationProgramKey=None).all()

    return render_template(
        'organization_requests.html',
        organization=organization,
        requests_with_devices=requests_with_devices,
        available_devices=available_devices
    )

@app.route('/delete_selected_devices/<int:donor_id>', methods=['POST'])
@login_required
def delete_selected_devices(donor_id):
    selected_device_ids = request.form.getlist('device_ids')  # Retrieve selected device IDs

    if selected_device_ids:
        for device_id in selected_device_ids:
            device = TrackerDonorDevices.query.get(device_id)
            if device:
                db.session.delete(device)  # Delete the device
        db.session.commit()  # Commit changes to the database
        flash('Selected devices have been deleted successfully.', 'success')
    else:
        flash('No devices selected for deletion.', 'warning')

    return redirect(url_for('donor_devices', donor_id=donor_id))
